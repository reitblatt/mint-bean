#!/usr/bin/env python3
"""
Create default rules that automatically categorize transactions based on Plaid categories.

This script creates rules that match on Plaid's detailed category and automatically
set the transaction to the corresponding beancount category.
"""
...
# Mapping of Plaid categories to beancount account names
...
def create_plaid_category_rules():
    """Create default rules for Plaid category mappings for all users."""
    print("=" * 70)
    print("Creating Plaid Category Auto-Categorization Rules")
    print("=" * 70)
    ...
    try:
        ...
        for user in users:
            ...
            for plaid_category, beancount_account in PLAID_CATEGORY_MAPPINGS.items():
                ...
                existing_rule = (
                    ...
                    .filter(
                        ...
                        Rule.name == f"Auto: {plaid_category}",
                    )
                    .first()
                )
                ...
                # Create the rule (convert dicts to JSON strings for SQLite)
                rule = Rule(
                    user_id=user.id,
                    name=f"Auto: {plaid_category}",
                    description=f"Automatically categorize transactions with Plaid category {plaid_category}",
                    conditions=json.dumps(
                        {
                            "plaid_detailed_category": {
                                "operator": "equals",
                                "value": plaid_category,
                            }
                        }
                    ),
                    actions=json.dumps(
                    ...
                    priority=10,  # Lower priority than custom rules (which default to 0)
                    active=True,
                )
        ...
        print("=" * 70)
        print(f"✅ Created {total_created} total rules across {len(users)} user(s)")
        if total_skipped > 0:
            print(f"⊘  Skipped {total_skipped} existing rules")
        ...
        if missing_categories:
            ...
            for cat in missing_categories:
                print(f"   - {cat}")
            print()
            print("   These categories need to be created first before rules can be added.")

        print("=" * 70)

    except Exception as e:
        db.rollback()
        print(f"\n❌ Failed to create rules: {e}")
        import traceback
